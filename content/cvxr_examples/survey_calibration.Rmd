...
Tmat <- apply(model.matrix(object = formula, data = apipop),
           2,
           sum)

cal_api <- calibrate(design_api, formula, population = Tmat, calfun = cal.raking)
...
w_survey_q <- weights(calibrate(design_api, formula, population = Tmat, calfun = cal.linear))
```
...
```{r}
u <- 1.10; l <- 0.90
w_survey_l <- weights(calibrate(design_api, formula, population = Tmat, calfun = cal.linear,
                                bounds = c(l, u)))
...
w_survey_h <- weights(calibrate(design_api, formula, population = Tmat, calfun = hellinger))
...
```{r}
w_survey_s <- weights(calibrate(design_api, formula, population = Tmat, calfun = cal.sinh,
                                bounds = c(l, u)))
