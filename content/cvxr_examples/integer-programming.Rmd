...
We can solve this problem as usual using the `ECOS` (actually
...
`CBC` and `GLPK` (via `GLPK_MI`) provided the respective R packages are installed
as documented in the tutorial [Using Other
Solvers](/cvxr_examples/using-other-solvers/).

```{r}
result <- solve(problem, solver = "CBC")
cbc_solution <- result$getValue(x)
...
```{r}
occupy <- Bool(length(people), length(offices))
```
